<?php

namespace Flutterping\Resources\Definitions;

class ElementDefinitions
{
    const Container = 'Container';

    const Text = 'Text';

    const Column = 'Column';

    const Row = 'Row';

    const Flex = 'Flex';

    const Image = 'Image';

    const Button = 'Button';

    const Divider = 'Divider';

    const Spacer = 'Spacer';

    const Border = 'Border';

    const BorderRadius = 'BorderRadius';

    const BorderSide = 'BorderSide';

    const BoxDecoration = 'BoxDecoration';

    const BoxFit = 'BoxFit';
    const Alignment = 'Alignment';
    const Repeat = 'Repeat';
    const FilterQuality = 'FilterQuality';
    const ImageNetwork = 'ImageNetwork';

    const Axis = 'Axis';

    const BoxShadow = 'BoxShadow';

    const DecorationImage = 'DecorationImage';

    const EdgeInsets = 'EdgeInsets';

    const EdgeInsetsGeometry = 'EdgeInsetsGeometry';

    const NetworkImage = 'NetworkImage';

    const TextStyle = 'TextStyle';

    const CrossAxisAlignment = 'CrossAxisAlignment';

    const MainAxisAlignment = 'MainAxisAlignment';

    const Color = 'Color';

    const FontWeight = 'FontWeight';

    const SizedBox = 'SizedBox';

    const EdgeInsetsPadding = 'EdgeInsetsPadding';

    const EdgeInsetsMargin = 'EdgeInsetsMargin';

    const AppBar = 'AppBar';

    const Scaffold = 'Scaffold';

    const StatelessWidget = 'StatelessWidget';

    const StatefulPage = 'StatefulPage';

    const MaterialPage = 'MaterialPage';

    const ElevatedButton = 'ElevatedButton';

    const IconData = 'IconData';

    const BottomNavigationBar = 'BottomNavigationBar';

    const BottomNavigationBarItem = 'BottomNavigationBarItem';

    const NavigationPath = 'NavigationPath';

    const ApiPath = 'ApiPath';

    const DynamicWidget = 'DynamicWidget';

    const ListView = 'ListView';
    const GridView = 'GridView';
    const SingleChildScrollView = 'SingleChildScrollView';

    const GestureDetector = 'GestureDetector';

    const CircularProgressIndicator = 'CircularProgressIndicator';

    const ReactiveWidget = 'ReactiveWidget';

    const TextFormField = 'TextFormField';

    const Form = 'Form';

    const MaterialApp = 'MaterialApp';

    const MaterialAppRouter = 'MaterialAppRouter';
    const SliverGridDelegateWithFixedCrossAxisCount = 'SliverGridDelegateWithFixedCrossAxisCount';
    const Card = 'Card';
    const RoundedRectangleBorder = 'RoundedRectangleBorder';
    const ScrollController = 'ScrollController';
    const BottomAppBar = 'BottomAppBar';
    const IntrinsicHeight = 'IntrinsicHeight';
}
